//인접 관계 선택자
//직접 선택자로 요소를 먼저 선택후 해당 요소와 가까이에 있는 요소를 선택

//부모(나를 감싸고 있는 요소)
$("li").parent().css("border","1px solid red");

//하위 요소. 선택자 뒤에 띄어쓰기를 통해 구분
...
$("#inner-wrap > h1").css("color","red").css("border","1px solid red");
...
$("#target + li").css("color","purple"); //next()는 동일
...
//범위 제한 전체형(이전) / 동생(다음) 요소
$("#target").prevUntil(".content0").css("color","red ")
$("#target").nextUntil(".content0").css("color","red ")

//모든 상위 요소 
$("#target").parents().css("color","red");
