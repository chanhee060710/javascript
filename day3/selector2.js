...
$("#menu li:odd").css("color","blue") //짝수.인덱스는 0부터 시작~

//nth-child(숫자n) 선택자: 선택한 요소의 무리중 지정된 숫자의 요소 선택
$("#menu li:nth-child(4n)").css("color","black");
$("#menu li:nth-child(2n+1)").css("color","blue");
//nth-last-of-type(숫자) 선택자: 선택한 요소의 무리중 지정된 숫자의 요소 중에서
...
//eq(index): 선택한 요소중 지정한 인덱스 요소만!
// lt(index): 선택한 요소중 지정한 인덱스 보다 작은 요소만
//gt(index): 선택한 요소 중 지정한 인덱스보다 큰 요소만

$("#menu li").eq(2).css("color","skyblue")
$("#menu li:eq(2)").css("color","skyblue") //[2]번째 요소
$("#menu li:lt(2)").css("color","salmon") //~[2]번쩨 여서 잔낒;
$("#menu li:gt(2)").css("color","violet") //[2]번재 요소 이후~
...
//obj라는 배열에서 area가 대전인 요소만 찾는 배열을 생성
//filter 기능과 비슷하지만, 기존데이터를 응용한 새로운 데이터를 생성할때good
//map 함수에 전달하는 공백함수(데이터,인덱스)(조건에 맞는 데이터 반환)
...
// ->존재한다면 해당 데이터의 인덱스값
//-> 없으면 -1 리턴
//$.isArray(): 찾고자 하는 데이터가 배열 객체인 true / 아니면 false
//$.merge(targetArr,subArr): 두배열을 하나의 객체로 결함
// ->targetArr에 subArr요소를 결함해주는 역할

const areas =["서울","대전","부산","전주"];
const countries =["한국","일본","스페인","프랑스"];
const tmpObj = {name:"찬희"};
